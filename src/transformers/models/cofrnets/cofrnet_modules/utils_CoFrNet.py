...
class MinMaxClipper(nn.Module):
    ...
    def __init__(self, width):
        ...
        self.width = width
        self.initval = torch.inf
        # Initialize buffers for minimum and maximum values
        self.register_buffer("min", torch.full((self.width,), self.initval))
        self.register_buffer("max", torch.full((self.width,), -self.initval))
    ...
    def reset_min_max(self):
        ...
        """
        with torch.no_grad():
            self.min = torch.full_like(self.min, self.initval)
            self.max = torch.full_like(self.max, -self.initval)
