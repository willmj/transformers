...
from .utils_CoFrNet import generate_connections
from .utils_CoFrNet import modified_reciprocal_activation, MinMaxClipper
...
class CoFrContinuant(torch.autograd.Function):
    ...
    def forward(ctx, a, b=None, epsilon=0.01):#, mask=None, output_direct=None):
        ...
        #Uncomment below 3 lines if no division (and comment above 2 lines)
        denom_clipped = torch.clamp(torch.abs(K[:, :, :, 0]), min=epsilon)
        output = torch.sign(K[:, :, :, 1])*torch.sign(K[:, :, :, 0])*\
                torch.exp(torch.log(torch.abs(K[:, :, :, 1])) - torch.log(denom_clipped))
        ...
        ctx.save_for_backward(b, K) #Uncomment if no division and comment above line
    ...
    def backward(ctx, grad_output):
        ...
        b, K = ctx.saved_tensors #Uncomment if no division and comment above line
        grad_a = grad_b = None
        ...
        if ctx.needs_input_grad[0]:
            ...
            #Uncomment below 3 lines if no division and comment above line
            K0_e = K[:, :, :, 0].unsqueeze(-1)
            denom_clipped = torch.clamp(torch.abs(K0_e), min=0.01)
            K_ratio = torch.sign(K)*torch.sign(K0_e)*torch.exp(torch.log(torch.abs(K)) - torch.log(denom_clipped))
